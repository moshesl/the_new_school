...
def students():
    ...
    if request.method == 'POST':
        ...
        return jsonify(db_students.add_stu(name, t_z, birthday, class_))
...
def student(stu_id):
    ...
    if request.method == 'GET':
        lst = db_students.get_students_year(stu_id)
        return jsonify(lst)
    # מסיר תלמיד
    if request.method == 'DELETE':
        return jsonify(db_students.remove_stu(stu_id))
...
def update_student(stu_id):
    ...
    json_dict = request.get_json()
    db_students.update_stu(stu_id, json_dict)
...
def get_subjects_by_year(stu_id, year):
    ...
    fetch = db_students.subject_per_year(stu_id, year)
...
def get_test(stu_id, year, sub_id):
    #  מחזיר רשימת מבחנים
    if request.method == 'GET':
        return jsonify(db_students.stu_subject(stu_id, sub_id, year))
    # מוסיף מבחן
    if request.method == 'POST':
        fech = request.get_json()
        day = fech['day']
        return jsonify(db_students.insert_test(sub_id, year, day))


@app.route('/students/<int:stu_id>/<int:year>/all_sub_avg/', methods=['GET'])
def get_avg_of_year(stu_id, year):
    # ממוצע כללי של כל המקצועות
    return jsonify(db_students.avg_all_grade(stu_id, year))
...
def updte_by_test_id(stu_id, year, sub_id, test_id):
    ...
    if request.method == 'PUT':
        ...
        return jsonify(db_students.update_grade(test_id, stu_id, new_grade))
    #     מוסיף שורה לטבלת ציונים
    if request.method == 'POST':
        fech = request.get_json()
        class_ = fech['class']
        grade = fech['grade']
        return jsonify(db_students.insert_student_grade(stu_id, sub_id, year, grade, class_, test_id))
    if request.method == 'GET':
        # מחזיר מבחן ספציפי
        return jsonify(
            db_students.specific_test_grade(test_id, stu_id))


@app.route('/students/<int:stu_id>/<int:year>/<int:sub_id>/avg/', methods=['GET'])
def stu_by_year_sub_test(stu_id, year, sub_id):
    # מחזיר ממוצע של מקצוע מסויים
    return jsonify(db_students.avg_stu_sub(sub_id, year, stu_id))
...
def subject_by_stu_all_years(stu_id, sub_id):
    ...
    if request.methods == 'GET':
        return jsonify(db_students.avg_stu_sub_all_years(sub_id, stu_id))
    # ?
...
def get_avg_of_years(stu_id, specific_years):
    return jsonify(db_students.avg_all_grade_few_years(stu_id, specific_years))
